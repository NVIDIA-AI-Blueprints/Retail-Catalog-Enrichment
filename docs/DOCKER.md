...
# Optional overrides used by both MinIO and Milvus
...
The policy RAG stack provides local-development MinIO credentials when the
variables are omitted and passes the same values to Milvus. Override both
values in `.env` before any shared or production-like deployment.
